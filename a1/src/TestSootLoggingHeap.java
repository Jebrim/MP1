		...
		if(!b.getMethod().getDeclaringClass().getName().equals("Log"))
		{
			...
			while(it.hasNext()){
				...
				if (stmt.containsFieldRef()) {
					...
					stmt.getDefBoxes().iterator().next().getValue();
					AssignStmt assignedStmt = (AssignStmt)stmt;
					Log.logFieldAcc(field.getDeclaringClass(), field.getName(), field.isStatic(), (assignedStmt.getRightOp() instanceof FieldRef) == false, assignedStmt.getRightOp());
					//if (stmt.getDefBoxes().iterator().hasNext())
					//{
					//	System.out.println(stmt.getDefBoxes().iterator().next().getValue());
					//}
				}
			}
		}
